#!/usr/bin/env bash
# Build + push the custom DataSpoke Airflow image.
# Matches the pattern in dev_env/dataspoke-api/build.sh.
set -euo pipefail
...
steps:
  - name: 'gcr.io/cloud-builders/docker'
    args: ['build', '-t', '${_IMAGE}', '-f', 'docker-images/airflow/Dockerfile', '.']
images: ['${_IMAGE}']
CLOUDBUILD
    ;;
  ...
  *)
    ...
    docker build -t "${IMAGE}" -f "${PROJECT_ROOT}/docker-images/airflow/Dockerfile" "${PROJECT_ROOT}"
    info "Pushing ${IMAGE}..."
    docker push "${IMAGE}"
    ;;
esac
