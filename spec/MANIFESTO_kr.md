...
- **전사 지표 시계열 모니터링**: 기존 메타데이터와 검증 결과로부터 메타데이터 건강도 신호 — 문서화 커버리지, 데이터 신선도 등 — 를 집계하는 대시보드.
- **다각도 데이터 조망**:
...
Claude Code 설정(`.claude/`)으로 구성된 AI 개발 환경이다. 프로젝트 구조, 네이밍 컨벤션, 스펙 체계, 운영 환경을 첫 세션부터 에이전트가 인지한다. 상세 사양은 `spec/AI_SCAFFOLD.md`를 참고한다.

- **스킬(Skills)** — 개발 환경 관리, 스펙 작성 및 동기화, DataHub 통합 가이드, Kubernetes 운영, 이슈 일괄 생성.
- **서브에이전트(Subagents)** — 구현 파이프라인. 표준 워크플로우: spec → api → backend → frontend → k8s/helm.
- **PR 자동화(PRauto)** — cron 기반 PR 자동화. `prauto:ready` 라벨의 GitHub 이슈를 감시하고, Claude Code CLI로 분석·구현·PR 제출을 자동화한다. 상세 사양은 `spec/AI_PRAUTO.md`를 참고한다.
- **맞춤형 솔루션 지원** — 포크 후 매니페스토, 스펙, 개발 환경을 조직에 맞게 수정하여 사용하도록 설계되었다.
