...
- **Online Verifier (실시간 검증)**: 데이터 카탈로그가 데이터 검증
  규칙을 실시간으로 등록하고 실행하는 API를 제공하고, 이것을 이용해
  코딩 에이전트가 데이터 파이프라인 개발을 위한 closed loop를 완성한다.
...
- **Baseline Product** — 자기 조직화, 자기 정화 및 실시간 검증
  기능을 가진 데이터 카탈로그의 기본 구현체
...
- **Ingestion Control**: 데이터 수집 설정, 제어 및 관리를 한곳에서
  수행할 수 있는 편의 기능 제공
- **Validation**: 시계열 규칙을 포함한 검증 규칙 등록/실행/관리.
  건식 검증(dry-run), 과거 시점 검증, 실시간 API 지원
- **Ontology**: 기본 데이터 문서(테이블 설명 등) 외에 소스
  코드(GitHub), SQL 로그, 외부 문서 등을 분석해 스스로 온톨로지를
  구성하고 그래프 DB 및 벡터 DB에 유지
- **Doc Generation**: 온톨로지에 기반해 데이터 문서화 상태를
  점검하고, 생성형 AI로 문서를 제안하는 API 및 검수 프로세스 제공
