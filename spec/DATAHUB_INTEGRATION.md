...
| `DATASPOKE_TEST_DATAHUB_TOKEN` | Personal access token; required because dev GMS runs with `METADATA_SERVICE_AUTH_ENABLED=true`. Generated by `helm-charts/bin/dev-peripherals/datahub.sh` via the frontend `/logIn` + `createAccessToken` flow and written back to `.env.dev`. | generated PAT |
