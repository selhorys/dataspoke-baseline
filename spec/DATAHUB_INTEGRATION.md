...
Two SDK clients serve different purposes. The app pod reads `gms_url` and `token` from the DB `peripheral_config` table (populated via `/api/v1/admin/peripherals/datahub`); test/dev tooling reads them from `DATASPOKE_DEV_DATAHUB_{GMS_URL,TOKEN}` in `helm-charts/.env`.
...
graph = DataHubGraph(DatahubClientConfig(
    ...
    token=token,         # peripheral_config.datahub.token   (app) | $DATASPOKE_DEV_DATAHUB_TOKEN   (tests)
))
...
side, and as `DATASPOKE_DEV_DATAHUB_*` env vars in `helm-charts/.env` on the test/dev tooling
...
|----------|---------|------------|
| `DATASPOKE_DEV_DATAHUB_GMS_URL` | GMS endpoint read by integration tests, `tests/integration/util/datahub.py`, and the `datahub-api` skill | Written by `helm-charts/bin/peripherals/datahub.sh` (`http://datahub.<INGRESS_DOMAIN>/gms`) |
| `DATASPOKE_DEV_DATAHUB_TOKEN` | Personal access token; required because dev GMS runs with `METADATA_SERVICE_AUTH_ENABLED=true`. Generated by `helm-charts/bin/peripherals/datahub.sh` via the frontend `/logIn` + `createAccessToken` flow and written back to `.env`. | generated PAT |
