...
tooling reads everything from `DATASPOKE_TEST_DATAHUB_*` env vars in `helm-charts/.env`
...
`helm-charts/bin/post-install/seed-peripheral-config.sh`). Non-secret fields `gms_url` and
`kafka_brokers` live in the DB; the `token` is routed to the `dataspoke-datahub-secret` K8s
...
| `DATASPOKE_TEST_DATAHUB_TOKEN` | Personal access token; required because dev GMS runs with `METADATA_SERVICE_AUTH_ENABLED=true`. Generated by `helm-charts/bin/dev-peripherals/datahub.sh` via the frontend `/logIn` + `createAccessToken` flow and written back to `.env`. | generated PAT |
