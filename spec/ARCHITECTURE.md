...
| Ontology Generation | UC3 | `/spoke/ontogen/` (singleton conf + Markdown seeds + node / edge / triple browse + review) | Dataset → Node Inference (embedding-based reuse + member-dataset mapping), Relationship Inference, Triple Composition, Review Queue (node + edge + triple) | LLM API, PostgreSQL (pgvector), Airflow (tier-based periodic DAG) |
