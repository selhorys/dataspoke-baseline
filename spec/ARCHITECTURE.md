...
|---------|-------------|
| Ingestion Control | Per-source ingestion in three modes — `DATAHUB_MANAGED` (DataHub's recipe + cron; DataSpoke syncs the definition + maps datasets), `ACTIVE_CUSTOM_MANAGED` (DataSpoke's pluggable/forkable extractor runs on a tier schedule), `PASSIVE` (external ingestor; DataSpoke records registration + declared scope and observes per-dataset ingestion). Source→dataset mapping makes all ingestion visible, including an unmanaged bucket |
| Validation | One validation slot per dataset — description + declared variable names (emitted as DataHub `assertionInfo`), plus a DataSpoke-side data-arrival cadence the Governance `validation-score` metric reads and opaque storage for the pipeline's own hyperparameters; ingestion of pipeline-emitted timeseries results emitted as `assertionRunEvent`; historical-result query (`from`/`until`) for use as a baseline cache. |
...
| Governance | Active metric aggregation (pure aggregation over DataHub metadata + DataSpoke results) with three built-in types — `ingestion-freshness`, `validation-score`, `doc-health` — each emitting named floating-point `values` plus a per-dataset breakdown over a configurable `dataset_filter` (a SQL `WHERE` clause over the dataset registry); scheduled per `schedule_tier` or run on demand. Every non-dry run also persists a per-dataset verdict, so the metric's covered datasets and their latest true/false/unknown criterion state are queryable directly. Passive ingestion of externally-computed metric results is reserved for a future release. |
...
|---|---|---|---|
| UC1 Ingestion Control | Airflow tier DAG (`ACTIVE_CUSTOM_MANAGED`) or the 2-hourly `datahub-sync-hourly` DAG (`DATAHUB_MANAGED`/`PASSIVE` sync); manual `POST .../sources/{id}/method/run` (`ACTIVE_CUSTOM_MANAGED` only) | `IngestionService` | `ACTIVE_CUSTOM_MANAGED`: pluggable extractor emits `Status` + `DatasetProperties` + `SchemaMetadata` plus per-run DPI aspects ([Custom Extractor Guide](DATAHUB_INTEGRATION.md#custom-extractor-guide)).<br>`DATAHUB_MANAGED`/`PASSIVE`: no aspect writes; syncs source defs, rebuilds the source→dataset mapping, and mirrors run history for `DATAHUB_MANAGED`.<br>The sweep's estate-wide `Dataset.lastIngested` observation runs for every mode, `ACTIVE_CUSTOM_MANAGED` included; the `Operation`-aspect observation stays `PASSIVE`-scoped. |
...
- All `dataset_filter`-bearing features (UC3 ontogen conf, UC4 metagen conf, UC5 metric
  definitions) share one filter grammar and one resolver: a SQL `WHERE` clause evaluated
  against `dataset_registry`, DataSpoke's local mirror of the DataHub estate, refreshed by
  the `datahub-sync-hourly` attribute sweep. Scope resolution is therefore a DataSpoke-side
  SQL query rather than a per-feature DataHub search, and is as fresh as the last sweep.
  Literal `dataset_urn` values matching no registered dataset surface on the corresponding
  `RUN_COMPLETE` event's `unresolved_urns` field.
...
| Ontology Generation | UC3 | `/spoke/ontogen/` (singleton conf + Markdown seeds + node / edge / triple browse + review) | Dataset → Node Inference (embedding-based reuse + member-dataset mapping), Edge (Predicate) Inference, Triple Composition, Confidence Scoring & Human Review Queue | LLM API, PostgreSQL (pgvector), Airflow (tier-based periodic DAG) |
| Metadata Generation | UC4 | `/spoke/metagen/conf` (conf collection: CRUD + per-conf `/method/run`, `/event`) + `/spoke/metagen/{item,dataset,event,uncovered}` (global review queue, per-dataset rollup, cross-conf events, uncovered datasets) + `/spoke/common/data/{urn}/attr/metagen/{boundary,item}` + `/event/metagen` | Metadata Generation Service, Producer-Reviewer Adversarial Debate, Per-Item Candidate Review Queue | LLM API, PostgreSQL (pgvector for `metagen_candidate_embeddings`), DataHub SDK (read context + approved writes to editable description aspects), Airflow (tier-based periodic DAGs fanning out across confs) |
| Governance | UC5 | `/spoke/governance/metric/` | Metrics Aggregator (pure aggregation, no source-DB reads), Built-in measurers (`ingestion-freshness`, `validation-score`, `doc-health`), Per-Dataset Breakdown and latest-verdict store, Factory-Default Bootstrap | Airflow (tier-based periodic DAGs + on-demand `metrics`), PostgreSQL (metric rows + the dataset registry a `dataset_filter` resolves against), DataHub GraphQL |
...
|---------|----------------|-----------|
| Airflow DAGs | Airflow | Periodic active-custom ingestion (UC1), DataHub-managed/passive source + mapping sync (UC1, the 2-hourly `datahub-sync-hourly`), ontology re-inference (UC3), metadata generation (UC4), governance metrics (UC5). Validation (UC2) is **not** scheduled by DataSpoke — pipelines POST results directly. |
