...
│   ├── pr-review.md            # [COMMITTED] Prompt template: address PR reviewer feedback
│   ├── feedback-response.md    # [COMMITTED] Prompt template: respond to plan feedback
...
2. For each candidate, fetch both inline review comments (`pulls/{N}/comments`) and issue-level comments (`issues/{N}/comments`). Before evaluating feedback, check the latest issue-level comment from the prauto actor — if it starts with `"Reviewer feedback addressed"`, check whether any non-prauto comments (issue-level or inline) have timestamps newer than the marker. If none exist, skip this PR. If newer comments exist, the PR is actionable again.
...
5. If a PR has actionable feedback (take the oldest PR first):
   ...
   c. Run the `pr-review` phase (same tool whitelist as implementation) with reviewer comments as context; the prompt (`prompts/pr-review.md`) instructs Claude to produce a reviewer-facing response summarizing changes and answering questions; save session output to `state/sessions/review-I-{N}.json`
   d. Push additional commits to the PR branch; call `create_or_update_pr` to add a commit-log comment to the existing PR
   e. Post a `"prauto(...): Review response"` comment with Claude's summary of changes and answers to reviewer questions via `post_review_response_comment`
   f. Post a `"prauto(...): Reviewer feedback addressed."` marker comment via `post_feedback_addressed_comment`
   g. Complete the job (move to history) and **exit the heartbeat**

The feedback-addressed marker is the key mechanism that breaks the re-pickup loop: once prauto addresses feedback and posts the marker, subsequent heartbeats see the marker and skip the PR. When a reviewer posts new comments after the marker, `find_actionable_prs()` detects comments with timestamps newer than the marker and treats the PR as actionable again.
...
### `prompts/pr-review.md` — PR review with response
...
This replaces the inline prompt previously hardcoded in `run_pr_review()`. The "Response" section is the key addition — it instructs Claude to produce a reviewer-facing summary that gets posted as a `"Review response"` comment on the PR. `CLAUDE_OUTPUT` (the `.result` field from Claude's JSON output) is captured as `REVIEW_RESPONSE`.

### `prompts/feedback-response.md` — Plan feedback response

```markdown
A reviewer provided feedback on a plan for GitHub issue #{number}: {title}.
Write a brief response addressing each point in the feedback.
...
1. Address each point directly.
2. If a question was asked, answer it.
3. If a suggestion was made, acknowledge it and state how it will be incorporated.
4. Keep the response under 500 words.
5. Do NOT include the revised plan — it will be posted separately.
6. Output only the response text.
```

This prompt is invoked with `--max-turns 1` and no allowed tools (text generation only), same pattern as the squash-commit prompt. The output is posted as a `"Feedback response"` comment on the issue before the revised plan is posted.
...
| Review response | `Review response` | `prauto(prauto01): Review response` |
| Feedback response | `Feedback response` | `prauto(prauto01): Feedback response` |
