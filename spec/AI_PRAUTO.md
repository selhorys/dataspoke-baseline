...
3. **Quota reset?** — the paused agent's probe passes → post "Resumed" and **resume** the same
   session (`--resume <session-id>`, same agent).
4. **Still down** — do nothing, exit; the pause never increments the retry counter.

The session id is generated by the harness upfront (`--session-id <uuid>`) so it is known
regardless of how the worker exits. Resuming is same-agent only: a session cannot migrate
agents, and agent-switch is reachable only through the abandon+restart path. The
`plan-approval` phase is exempt from resume — it waits on a human, not on agent quota.
...
> probe Codex (`~/.codex/auth.json` then a `codex exec "Reply with exactly: OK"` dry-run). Set
