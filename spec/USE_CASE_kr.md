...
Nodes (subjects / objects):
  ...
  각 행은 자신을 만든 run_id를 가지며, 모든 개념 뒤의 producer/reviewer 디베이트는
  그 실행의 Langfuse 세션에 기록된다.
...
"rating"과 "review"를 구분하는 데 모호함이 있음) 리뷰어가 노드부터 시작한다. LLM이
왜 이 개념을 제안했는지 보려면 리뷰어는 해당 행의 `run_id` Langfuse 세션을 여는데,
거기에 producer/reviewer 디베이트 기록 전체가 담겨 있다:
