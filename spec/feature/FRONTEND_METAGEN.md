...
| `/metagen/data/[urn]` | `GET .../attr/metagen/conf`, `GET .../attr/metagen/item`, `GET .../attr/metagen/item/{item_id}` (per-item candidates), `GET .../event/metagen` | `PUT/PATCH/DELETE .../attr/metagen/conf` (fields: `is_enabled`, `allowed[]`, `owner`); `POST .../attr/metagen/item/{item_id}/candidate/{candidate_id}/method/review` body `{verdict: "approve"\|"reject", reason}` |
...
The global page (`/metagen`) is the singleton conf editor plus a paginated
cross-dataset queue of pending items (filterable by `dataset_urn` text, `kind`
and `status` selects), rendering `field_path` and `candidates` count columns.
The per-dataset page (`/metagen/data/[urn]`) shows boundary
(`is_enabled`, `allowed`, `owner`) and the dataset's items grouped by kind.
Its header renders an enabled/disabled badge beside the dataset URN. Each item
...
Write actions — conf save, conf/boundary `Delete` (button → ConfirmDialog),
`Run`, boundary edits, `Approve`, `Reject` — are
