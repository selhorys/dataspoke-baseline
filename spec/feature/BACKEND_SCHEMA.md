...
| `parameter` | `JSONB` NULL | Optional pipeline hyperparameters — a JSONB array of `{name, value, description}` objects (one field wider than `variables`' `{name, description}`, sharing the same per-field string rule and 1–200 count bound), in its own name namespace. `NULL` when the section was never supplied, which is the distinct "not declared" state an empty array does not express (an explicit `[]` is rejected at the API schema layer). Opaque to DataSpoke: no service reads it and it is not emitted to DataHub |
| `created_at` | `TIMESTAMPTZ` | |
| `updated_at` | `TIMESTAMPTZ` | |
