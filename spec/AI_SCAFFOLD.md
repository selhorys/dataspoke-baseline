...
| `dev-env` | Dev environment management: configure, install (full or partial), uninstall (full or partial), start/stop port-forwarding, health-check, and run-dataspoke-test-mode (build + deploy in-cluster API via Helm). Accepts action + optional component/options as arguments |
