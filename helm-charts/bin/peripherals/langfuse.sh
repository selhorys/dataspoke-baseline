...
# Headless-init seeded credentials (dev).
...
# Langfuse admin password: auto-generate if unset or empty. The Langfuse UI is
# published via nip.io ingress and reachable from any host that can reach the
# GKE LoadBalancer IP — a hardcoded default password is a public-ingress risk.
if [[ -z "${DATASPOKE_DEV_LANGFUSE_INIT_USER_PASSWORD:-}" ]]; then
  _langfuse_pw="$(openssl rand -base64 24 | tr -d '/+=' | cut -c1-20)"
  upsert_env_var DATASPOKE_DEV_LANGFUSE_INIT_USER_PASSWORD "${_langfuse_pw}" "${ENV_FILE}"
  export DATASPOKE_DEV_LANGFUSE_INIT_USER_PASSWORD="${_langfuse_pw}"
  info "Generated Langfuse admin password; persisted to .env."
fi
...
# ---------------------------------------------------------------------------
# Create dataspoke-langfuse-secret in langfuse namespace (idempotent)
# ---------------------------------------------------------------------------
