...
echo "  POST   http://${INGRESS_IP}:9221/lock/acquire      # acquire (returns a token)"
echo "  POST   http://${INGRESS_IP}:9221/lock/renew        # extend the lease {owner, token}"
echo "  POST   http://${INGRESS_IP}:9221/lock/release      # release {owner[, token]}"
