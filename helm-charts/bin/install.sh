...
--set
frontend.config.apiBaseUrl=http://api.${domain}
--set
frontend.config.datahubUrl=http://datahub.${domain}
--set
frontend.config.langfuseUrl=http://langfuse.${domain}
--set-string
frontend.config.langfuseProjectId=${DATASPOKE_DEV_LANGFUSE_INIT_PROJECT_ID:-dataspoke-project}
--set
frontend.config.airflowUrl=http://airflow.${domain}
EOF
}
...
NEXT_PUBLIC_API_BASE_URL=http://api.${domain}
NEXT_PUBLIC_DATAHUB_URL=http://datahub.${domain}
NEXT_PUBLIC_LANGFUSE_URL=http://langfuse.${domain}
NEXT_PUBLIC_LANGFUSE_PROJECT_ID=${langfuse_project_id}
NEXT_PUBLIC_AIRFLOW_URL=http://airflow.${domain}
EOF
  info "Wrote ${env_local_path} (API: http://api.${domain}, DataHub: http://datahub.${domain}, Langfuse: http://langfuse.${domain} [project: ${langfuse_project_id}], Airflow: http://airflow.${domain})"
}
...
if [[ "$PROFILE" == "dev" ]]; then
  ...
  if [[ "${#COMPONENTS[@]}" -eq 1 && "${COMPONENTS[0]}" == "api" ]]; then
    ...
    if [[ -n "$DOMAIN" ]]; then
      ...
      elif curl -sf -X POST "http://api.${DOMAIN}/internal/admin/dags/verify" \
    ...
    if [[ -n "${DATASPOKE_KUBE_INGRESS_DOMAIN:-}" ]]; then
      echo "  API:   http://api.${DATASPOKE_KUBE_INGRESS_DOMAIN}/api"
      echo "  ReDoc: http://api.${DATASPOKE_KUBE_INGRESS_DOMAIN}/redoc"
    fi
    echo ""
  ...
  if [[ "${#COMPONENTS[@]}" -eq 1 && "${COMPONENTS[0]}" == "frontend" ]]; then
    ...
    echo "  API:      http://api.${DOMAIN}/api/v1/"
    echo ""
  ...
  echo "  DataSpoke API: http://api.${DATASPOKE_KUBE_INGRESS_DOMAIN:-<not set>}/api/v1/"
  ...
  case "$FRONTEND_MODE" in
    ...
    local)
      ...
      echo "    src/frontend/.env.local written (API: http://api.${DATASPOKE_KUBE_INGRESS_DOMAIN:-<not set>}, DataHub: http://datahub.${DATASPOKE_KUBE_INGRESS_DOMAIN:-<not set>})"
