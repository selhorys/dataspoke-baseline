...
- **Self-hosted Langfuse** for LLM observability — sibling subsystem in its own `langfuse-01` namespace; consumed via `DATASPOKE_LANGFUSE_*` env vars (absence disables tracing). See `spec/feature/BACKEND_LLM.md §Observability`.
