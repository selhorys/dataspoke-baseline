...
// STUB-MODE ARC
//
// One ordered, stateful scenario (conf → seed → run → result → cleanup). Scoped in
// its own describe so a failure here aborts THIS arc only: the real-LLM arc below
// shares none of its module state and stands or falls on its own.
// ═══════════════════════════════════════════════════════════════════════════════
test.describe("UC3 — stub-mode arc", () => {
  // Serial mode: each step depends on the conf + seed + module state the prior step
  // established, so the arc's tests run as one group and a failing step aborts the
  // rest — a broken step reports as the failure it is instead of leaving the dependent
  // steps to run against inconsistent state.
  //   - retries: 0 → overrides the project-level `retries: 1`. Step 2 creates the seed
  //     by POSTing it and does NOT pre-delete by natural key (a seed carries no name to
  //     key on), so a group retry appends a SECOND seed with the same body. Step 2 then
  //     resolves the seed id with a preview `find`, which returns the FIRST match in list
  //     order — so the retry can bind the arc to the previous attempt's seed, and step 5
  //     deletes that one while the duplicate is left behind. Either way a retry litters a
  //     stray seed into the next run's inference set. This arc therefore takes the spec's
  //     other option and fails loudly in place.
  ...
  // ── Arc state shared across this arc's serial step tests ─────────────────────
...
test.describe("UC3 — real-LLM arc", () => {
  // Same stance as the stub arc: one ordered, stateful scenario run as a serial group.
  //   - retries: 0 → overrides the project-level `retries: 1`; real-LLM step 2 creates
  //     its seed with no pre-delete by natural key, so a group retry appends a second
  //     identical seed. Its preview `find` returns the FIRST match in list order, so the
  //     retry can bind to the previous attempt's seed and leave the duplicate behind —
  //     either way a stray seed survives into the next run's inference set.
  // spec: spec/TESTING.md §E2E §Execution discipline — "Ordered scenarios run serial…
  // Serial mode also states its retry stance: Playwright retries a failed serial group
  // from the first step, so a file either makes every step re-runnable or sets
  // `retries: 0` and fails loudly in place."
  ...
  // Real-LLM arc state (independent from the stub arc's state).
  ...
  test("UC3 real-LLM step 5 — cleanup: delete seed + disable conf", async ({ page, adminApi }) => {
    ...
    // removes, expressed as a silent no-op instead of a skip. Under mode: "serial" this
    // step does not run unless real-LLM step 1 (which sets realConfCreated only after
    // asserting the PUT status) and step 2 (which asserts the seed id truthy) both
    // passed, so a false backstop here means the arc state was lost, not that a step
    // legitimately produced nothing.
    // spec: spec/TESTING.md §Assertion Discipline — "Any guarded assert must be paired
    //   with a backstop that proves the guarded path executed … so the test fails when
    //   the value is absent instead of skipping silently."
    expect(realSeedId, "real-LLM step 2 must have created a seed").toBeTruthy();
    expect(realConfCreated, "real-LLM step 1 must have enabled the conf").toBe(true);
    ...
    // exact — the row container is itself role="button" whose aggregated name
    // contains "Delete"; only the actual button's name is exactly "Delete".
    // (Same locator as the stub arc's step 5; without exact it resolves to 2 elements
    ...
    realSeedId = null; // Mark cleaned up so afterAll does not double-delete.
