...
| `/ingestion/conf` | `uc1-02-active-custom-postgres.spec.ts` step 7; `uc1-03-passive-kafka.spec.ts` step 7; `uc1-01-datahub-managed.spec.ts` step 3 (list view, DataHub-managed filter; wrappers hidden) | `ground/shell/reader-write-suppressed.reader.spec.ts` (Reader read-only: "Create source" absent); `ground/ingestion/source-list-run-status.spec.ts` (status column reports the newest **run outcome**, not the head of the feed — a seeded failed run followed by a source edit, so a newer `INGESTION.SOURCE_UPDATE` with `status="success"` sits above the `INGESTION.FAIL`; dual-confirmed against `GET /spoke/ingestion/sources/{id}/event`) |
...
| Route | UC test | Ground test |
|---|---|---|
| `/governance/dashboard` | `_smoke.spec.ts` (post-login landing); `uc5-01-governance.spec.ts` step 3a (combined metric cards: title + `metric_type` badge + `description` + `Details` link to `/governance/metrics/{id}` + trend chart whose line strokes are the `metrics[]` descriptors' colors, in `idx` order) | `ground/shell/root-redirect.spec.ts` (`/` redirect target); `ground/governance/dashboard-view-controls.spec.ts` (view controls: all types checked / blank search / ascending title order by default; deselecting one type drops its cards; case-insensitive substring title search, inactive while blank and blind to `description`; deselecting every type empties the grid into the view-controls empty state, not the enable-a-metric one; the three choices survive a reload; and none of them changes the metric-list read — every `GET /spoke/governance/metric` carries exactly `is_enabled=true` + `limit=100`) |
| `/governance/metrics` | `uc5-01-governance.spec.ts` step 3b (list: metrics, type badges, Enabled) | — |
| `/governance/metrics/new` | `uc5-01-governance.spec.ts` step 1a (create form → redirect; per-key series rows — checkbox + `#RRGGBB` + display order — and the `dataset_filter` SQL clause box, both read back over REST as `{name, color, idx}` descriptors and a verbatim clause) | — |
| `/governance/metrics/[id]` | `uc5-01-governance.spec.ts` steps 1c, 2, 3c, 4 (Edit→PUT, Run, Config+Result+**Datasets**+Event, series-descriptor lines, `dataset_filter` clause, Delete) and the *dataset_filter worked examples* scenario (author a composite clause in the shape of the API.md worked example in the SQL editor, **Auto-indent** lays it out and the formatted text is what persists; an out-of-grammar clause renders its `422 INVALID_DATASET_FILTER` inline with the reported character position and leaves the stored clause untouched; run → Datasets panel rows + `met` toggle narrowing, dual-confirmed against `GET .../dataset`) | `ground/governance/metric-grain.spec.ts` (Result panel ChartGrainPicker: Hourly/Daily/Weekly with Daily default; switching grain adds no request parameter — no `grain` in the `attr/result` query and the sent query strings unchanged; the choice survives a reload and carries to a second metric's Result panel — one stored grain per panel type); `ground/governance/metric-datasets-panel.spec.ts` (Datasets panel: the four columns and a never-run metric's `unknown` rows with em-dash check times, `dataset_urn` → `/data/[urn]` and the DataHub deep-link; the toggle group's visible `criterion met:` label, which leaves each checkbox's accessible name its own verdict; the three-way toggle group drives the repeatable `met` param on the wire, zero toggles issue **no request** and render the empty state, re-checking one restores the table; the muted `attrs_synced_at` scope-freshness line) |
...
| `/metagen/conf/new` | `uc4-01-metadata-generation.spec.ts` step 2 (create conf EU + conf OE → redirect) | `ground/metagen/conf-new.spec.ts` (fill form → POST → redirect → backend read-back of fields + the `dataset_urn = '…'` clause typed into the shared SQL `dataset_filter` editor, stored verbatim) |
...
| A multi-line `dataset_filter` clause renders with its indentation **visually intact** (not collapsed) in `DatasetFilterView`, on the metric detail and the OntoGen / MetaGen conf views. `src/frontend/components/dataset-filter-view.test.tsx` and the conf-view / metric-detail page tests compare the `<pre>`'s raw `textContent` and assert the declared `whitespace-pre*` / `font-mono` classes, but jsdom loads no stylesheet, so the class is a proxy for the rendered result. | `spec/feature/FRONTEND_BASIC.md §Shared component notes → DatasetFilterView` ("a monospace `<pre>` block preserving the stored line breaks and indentation") | Needs a real browser to observe layout. Planned shape: a `ground/governance/` test that PUTs a metric whose clause carries an Auto-indented body, opens the detail view, and asserts the block's `getComputedStyle(...).whiteSpace` preserves the leading spaces (or compares the rendered height against a single-line control). |
