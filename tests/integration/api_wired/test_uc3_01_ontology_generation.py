...
Concerns covered (1 test function, parametrized over 2 values = 2 test cases):

  test_uc3_ontology_generation (parametrized: stub, real)

One test mirrors the UC3 user-story arc; the ``llm_mode`` parameter selects which
LLM the arc runs against, and exactly one of the two cases runs per dev-env
configuration (the other skips on the runtime conf's stub_llm_client).  The arc:
a DG operator enables ontology generation, seeds domain knowledge, runs real
(non-dry-run) inference, inspects the concept graph, opens the run's Langfuse
session (each row links to its creating run via run_id), then cleans up.
...
@pytest.mark.asyncio
@pytest.mark.parametrize("llm_mode", ["stub", "real"])
async def test_uc3_ontology_generation(
    llm_mode: str,
    api_client: httpx.AsyncClient,
    admin_headers: dict[str, str],
    runtime_conf: dict,
) -> None:
    """UC3 user-story arc, run against the stub LLM or a real LLM.

    Parametrized over llm_mode in ['stub', 'real'] — the dev-env's runtime conf
    decides which case runs and which skips, so the arc always executes against
    the LLM its case names:
      - 'real' skips when stub_llm_client is true (a stub LLM cannot satisfy the
        real-LLM contract: ≥1 persisted row stamped with this run's run_id).
      - 'stub' skips when stub_llm_client is false (the case would otherwise run
        the arc against a real LLM under a stub label, duplicating the 'real'
        case minus its non-zero-rows backstop).
    spec: TESTING.md §Running — both cases guard inline on
    runtime_conf['stub_llm_client'] as the first statement of the test body;
    exactly one case runs per dev-env configuration.
    ...
    Steps mirror USE_CASE_en.md §UC3 Imazon Example:
      ...
      5. GET result/{node,edge,triple} — assert standard envelope shape for each, and
         that every persisted row carries run_id (the row's link to its creating run's
         Langfuse session); track any_rows_found across all three result types
      6. Real mode only: assert any_rows_found is True — a real LLM run that persists
         zero rows carrying this run's run_id signals a prompt/filter regression.
         Under stub mode the Producer returns an empty payload, so this run persists
         no rows and the invariant does not apply.
    ...
    """
    if llm_mode == "real" and runtime_conf.get("stub_llm_client"):
        pytest.skip(
            "stub_llm_client=true; set stub_llm_client=false via PATCH /admin/conf "
            "to run real-LLM tests"
        )
    if llm_mode == "stub" and not runtime_conf.get("stub_llm_client"):
        pytest.skip(
            "stub_llm_client=false; the [real] case covers this arc against the real "
            "LLM. Set stub_llm_client=true via PATCH /admin/conf to run stub tests"
        )
    ...
    try:
        ...
        # The stub Producer ignores the seed body, so the stub case posts the rich
        # prose seed a DG operator would actually write; the real case posts a terse
        # concept list that keeps real-LLM inference within the run timeout.
        # spec: USE_CASE_en.md §UC3 §Imazon Example
        seed_md = {
            "stub": (
        ...
        assert run_complete is not None, (
            ...
            "spec: BACKEND_LLM.md §Evidence — per-run debate outcome is queryable from "
            "the run's events; the ONTOGEN.RUN_COMPLETE event detail carries "
            "debate_outcome and producer_iterations"
        )
        ...
        for result_type, list_key in [
            ...
            # Langfuse session (where the debate transcript lives). Rows this run produced
            # carry run_id == the RUN_COMPLETE event's run_id; rows from prior runs /
            # seeded fixtures carry a different id or NULL, so the equality to THIS run's
            # id is only meaningful for rows this run produced. any_rows_found is the
            # discriminating signal for real mode (Step 6) — a NULL or swapped run_id on
            # the new rows leaves no row matching this run's id and fails that assertion.
            # Under stub mode the Producer returns an empty payload, so this run persists
            # no new rows; the schema contract is asserted on whatever rows exist.
            # spec: BACKEND_LLM.md §Evidence — the run's Langfuse session —
            # row.run_id = session_id; the transcript lives in Langfuse, not in the row.
            for row in list_body[list_key]:
                ...
                # spec: API.md §Ontology Generation — GET result/{node,edge,triple}
                # each row carries `confidence_score`, `status`, `created_at`, `run_id`.
                assert "confidence_score" in row, (
                    f"{result_type} {row['id']!r} missing confidence_score field. "
                    "spec: API.md §Ontology Generation — each row carries "
                    "confidence_score, status, created_at, and run_id"
                )
                assert "status" in row, (
                    f"{result_type} {row['id']!r} missing status field. "
                    "spec: API.md §Ontology Generation — each row carries "
                    "confidence_score, status, created_at, and run_id"
                )
        ...
        # ── Step 6: Real mode only — the run must have produced rows ─────────
        # Under stub mode the Producer returns an empty payload, so zero rows persist
        # and this invariant does not apply.
        # spec: BACKEND_LLM.md §Test Mode — stub Producer returns an empty payload
        if llm_mode == "real":
            assert any_rows_found, (
                "Real LLM run produced zero rows carrying this run's run_id — verify "
                "prompt/filter pipeline. This is the backstop that makes Step 5's "
                "per-row checks non-vacuous: with zero rows the loop body never runs. "
                "spec: TESTING.md §Assertion Discipline — a guarded assert must be "
                "paired with a backstop that proves the guarded path executed. "
                "spec: BACKEND_LLM.md §Termination — 'persist each row tagged with "
                "the run_id'"
            )
