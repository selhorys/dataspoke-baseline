...
async def test_uc4_metadata_generation_with_real_llm(
    ...
    try:
        ...
        eu_desc_candidate = await _first_llm_approved_candidate(_EU_ENCODED, eu_desc_item_id)
        approved_eu_desc_value: str | None = None
        # Backstop (§Assertion Discipline — guarded asserts need a backstop): a real
        # LLM MUST predict an llm_approved candidate for eu_profiles dataset.description
        # (its description was masked in step 2), so an empty-LLM run fails loudly here
        # instead of vacuously skipping the approve assertions below.
        # spec: USE_CASE_en.md §UC4 §Imazon Example — step 8 approve dataset.description
        assert eu_desc_candidate is not None, (
            "Real-LLM run produced no llm_approved candidate for eu_profiles "
            "dataset.description (masked in step 2); step 8a approve cannot be verified. "
            "spec: USE_CASE_en.md §UC4 §Imazon Example — review approve dataset.description"
        )
        if eu_desc_candidate is not None:
            eu_desc_cid = eu_desc_candidate["candidate_id"]
            approved_eu_desc_value = eu_desc_candidate["value"]
        ...
        eu_email_candidate = await _first_llm_approved_candidate(_EU_ENCODED, eu_email_item_id)
        rejected_eu_email_cid: str | None = None
        # Backstop (§Assertion Discipline): the email column was masked in step 2, so a
        # real LLM MUST predict an llm_approved candidate for it; an empty-LLM run fails
        # here instead of vacuously skipping the step-8b reject assertions.
        # spec: USE_CASE_en.md §UC4 §Imazon Example — step 8 reject column.email.description
        assert eu_email_candidate is not None, (
            "Real-LLM run produced no llm_approved candidate for eu_profiles "
            "column.email.description (masked in step 2); step 8b reject cannot be "
            "verified. spec: USE_CASE_en.md §UC4 §Imazon Example — review reject email"
        )
        if eu_email_candidate is not None:
            rejected_eu_email_cid = eu_email_candidate["candidate_id"]
            eu_email_encoded_item = urllib.parse.quote(eu_email_item_id, safe="")
        ...
        # Backstop (§Assertion Discipline): step 2 must have masked at least one
        # orders.events column, otherwise the LLM has no OE target to predict and
        # step 8c would be silently skipped.
        # spec: USE_CASE_en.md §UC4 §Imazon Example — step 2 mask orders.events columns
        assert masked_oe_field_paths, (
            "No orders.events column was masked in step 2; the real-LLM OE prediction "
            "and step-8c approve cannot be exercised. "
            "spec: USE_CASE_en.md §UC4 §Imazon Example — mask orders.events columns"
        )
        if masked_oe_field_paths:
            oe_col_item_id = f"column.{masked_oe_field_paths[0]}.description"
            approved_oe_item_id = oe_col_item_id
            oe_col_candidate = await _first_llm_approved_candidate(_OE_ENCODED, oe_col_item_id)
            # Backstop (§Assertion Discipline): the masked OE column is a required LLM
            # target, so a real run MUST produce an llm_approved candidate for it.
            # spec: USE_CASE_en.md §UC4 §Imazon Example — step 8 approve orders.events column
            assert oe_col_candidate is not None, (
                "Real-LLM run produced no llm_approved candidate for the masked "
                f"orders.events column {oe_col_item_id!r}; step 8c approve cannot be "
                "verified. spec: USE_CASE_en.md §UC4 §Imazon Example — approve OE column"
            )
            if oe_col_candidate is not None:
                oe_col_cid = oe_col_candidate["candidate_id"]
                approved_oe_value = oe_col_candidate["value"]
