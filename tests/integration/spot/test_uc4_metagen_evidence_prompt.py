...
spec: spec/feature/BACKEND.md §Metadata Generation Service (Generation Pipeline, step 3 — evidence
sources incl. related documents + per-dataset ontology RAG)
...
async def _poll_until_document_indexed(
    ...
    *,
    ...
    spec: spec/feature/BACKEND.md §Metadata Generation Service (Generation Pipeline, step 3 —
    evidence sources incl. related documents + per-dataset ontology RAG)
    """
...
async def test_uc4_evidence_reaches_producer_prompt(
    ...
    spec: feature/BACKEND.md §Metadata Generation Service (Generation Pipeline, step 3 — evidence
    reaches the Producer prompt)
    """
    ...
    try:
        ...
        assert doc_title in prompt, (
            ...
            "spec: feature/BACKEND.md §Metadata Generation Service (Generation Pipeline step 3) — "
            "related_documents wired to prompt"
        )
        ...
        assert f"distinctive_{suffix}" in prompt, (
            ...
            "spec: feature/BACKEND.md §Metadata Generation Service (Generation Pipeline step 3) — "
            "related_documents body forwarded to LLM"
        )
        ...
        assert node_name in prompt, (
            ...
            "spec: feature/BACKEND.md §Metadata Generation Service (Generation Pipeline step 3) — "
            "per-dataset ontology RAG wired to prompt"
        )
