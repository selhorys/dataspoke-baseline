"""Unit tests for metrics workflow: flow IDs, YAML generation, and sync logic.

Tests cover:
- FLOW_ID and PERIODIC_FLOW_PREFIX constants
- schedule_to_flow_id() stability and prefix
- generate_periodic_flow_yaml() valid YAML structure and Kestra expression passthrough
- sync_periodic_metrics_flows() create/delete/unchanged logic
"""
...
def test_schedule_to_flow_id_deterministic_hash():
    ...
    cron = "0 3 * * *"
    expected = PERIODIC_FLOW_PREFIX + hashlib.md5(cron.encode()).hexdigest()[:8]
...
def test_generated_yaml_has_list_metrics_task(sample_flow):
    tasks = sample_flow["tasks"]
    list_task = next(t for t in tasks if t["id"] == "list_metrics")
    assert list_task["type"] == "io.kestra.plugin.core.http.Request"
    assert list_task["method"] == "POST"
    assert "/internal/activities/metrics/list-periodic" in list_task["uri"]


def test_generated_yaml_list_metrics_body_contains_schedule(sample_flow):
    tasks = sample_flow["tasks"]
    list_task = next(t for t in tasks if t["id"] == "list_metrics")
...
def test_generated_yaml_run_metric_task_exists(sample_flow):
    tasks = sample_flow["tasks"]
    each_task = next(t for t in tasks if t["id"] == "run_each")
    run_task = next(t for t in each_task["tasks"] if t["id"] == "run_metric")
    assert run_task["type"] == "io.kestra.plugin.core.http.Request"
    assert run_task["method"] == "POST"
    assert "/internal/activities/metrics/run" in run_task["uri"]


def test_generated_yaml_run_metric_has_retry(sample_flow):
    tasks = sample_flow["tasks"]
    each_task = next(t for t in tasks if t["id"] == "run_each")
    run_task = next(t for t in each_task["tasks"] if t["id"] == "run_metric")
...
def test_generated_yaml_kestra_expressions_survive_formatting():
    ...
    assert "{{ outputs.list_metrics.body }}" in raw
...
async def test_sync_creates_flow_for_active_cron_schedule(mock_db, mock_kestra):
    ...
    result = await sync_periodic_metrics_flows(
...
@pytest.mark.asyncio
async def test_sync_deletes_stale_metrics_periodic_flows(mock_db, mock_kestra):
...
async def test_sync_keeps_flow_matching_active_cron_as_unchanged(mock_db, mock_kestra):
    ...
    # create_or_update raises to simulate failure (flow appears in unchanged)
