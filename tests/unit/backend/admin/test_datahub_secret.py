...
Covers the DataHub token accessor — get, set, cache, fallback, and security invariants.
...
8.  get — out-of-cluster fallback: _require_client raises → returns settings.datahub_token
...
13. datahub_token_is_set: True when non-empty, False when "".
14. Secret name and key constants: _SECRET_NAME="dataspoke-datahub-secret", _SECRET_KEY="token".
...
def test_get_falls_back_to_settings_when_out_of_cluster(monkeypatch) -> None:
    """When _require_client raises SecretResolverUnavailable, returns settings.datahub_token.

    spec: plan/scalable-beaming-hamster.md §Backend — out-of-cluster fallback to env var (host-mode).
    """
    monkeypatch.setattr("src.backend.admin.datahub_secret.settings.datahub_token", "env-token")
    ...
    assert result == "env-token", (
        "Out-of-cluster fallback must return settings.datahub_token"
    )
...
def test_get_out_of_cluster_fallback_does_not_cache(monkeypatch) -> None:
    ...
    def _require_side_effect():
        ...
        secret = _fake_secret("incluster-token")
        core = _make_core(read_return=secret)
        return core, _NAMESPACE

    with patch("src.backend.admin.datahub_secret._require_client", side_effect=_require_side_effect):
        first = get_datahub_token()
        second = get_datahub_token()

    assert first == "env-token"
    assert second == "incluster-token", (
...
def test_set_out_of_cluster_raises() -> None:
    """set_datahub_token propagates SecretResolverUnavailable when out-of-cluster.
