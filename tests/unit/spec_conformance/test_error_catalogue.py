...
A fifth catalogue — the mapping block in ``exceptions.py``'s **module** docstring —
duplicates the class blocks and is checked against them rather than trusted.
...
from ._api_md import (
    ...
    module_docstring_exception_mapping,
    src_python_roots,
)
...
# or suppressed, and tagged with the issue-#86 phase that resolves it (1 = API-centric,
# 2 = backend, 3 = frontend). `assert_drift_allowlist` fails on a stale entry as well as
# an undeclared one, so an entry whose mismatch is resolved must be deleted here.
...
KNOWN_DRIFT_DOCSTRING_NOT_IN_API_MD: frozenset[str] = frozenset(
    {
        # `BadRequestError.error_code` defaults to "BAD_REQUEST" and the API maps
        # BadRequestError to HTTP 400 (src/api/main.py::_handle_bad_request), so the
        # code is reachable, but spec/API.md §Application Error Codes carries no row for
        # it — unlike INTERNAL_ERROR, the equivalent DataSpokeError default, which is
        # catalogued. Also declared as a class attribute, hence the twin entry in
        # KNOWN_DRIFT_CLASS_ATTR_NOT_IN_API_MD: both go when the API.md row lands.
        # Phase 1 (API-centric).
        "BAD_REQUEST",
        # INGESTION_DISABLED has no raise site and no spec/API.md row. It is kept —
        # rather than deleted alongside GENERATION_RUNNING / GENERATION_DISABLED /
        # INGESTION_NOT_APPLICABLE, which showed the same surface symptoms — because it
        # is the one missing member of a live, spec'd family: METRIC_DISABLED,
        # ONTOGEN_DISABLED and METAGEN_DISABLED all exist and are catalogued at 409 with
        # identical "is_enabled=false; non-dry-run rejected" semantics, and
        # `POST /spoke/ingestion/sources/{id}/method/run` is the ingestion sibling of
        # those run endpoints. The deleted three named a rename that had already
        # happened (GENERATION_* → METAGEN_*) or a concept superseded by
        # INGESTION_RUN_NOT_APPLICABLE, so they described nothing that exists. This one
        # describes a gap to close. Phase 2 (backend).
        "INGESTION_DISABLED",
    }
)
...
KNOWN_DRIFT_CLASS_ATTR_NOT_IN_API_MD: frozenset[str] = frozenset(
    {
        # See KNOWN_DRIFT_DOCSTRING_NOT_IN_API_MD above. Phase 1 (API-centric).
        "BAD_REQUEST",
        # NotificationError and EventProcessingError are DataSpokeError subclasses with
        # no dedicated handler, so `_handle_dataspoke_generic` returns them to the client
        # as HTTP 500 carrying these codes — but spec/API.md §Application Error Codes
        # lists neither, so a client cannot look either up. Both are live: raised at
        # src/shared/notifications/service.py:106 and src/shared/datahub/events.py:67,80.
        # Phase 1 (API-centric).
        "NOTIFICATION_FAILED",
...
#: docstring block, which claims to enumerate the valid values and does not.
KNOWN_DRIFT_ENTITY_TYPE_UNDECLARED: frozenset[str] = frozenset(
    {
        # All seven are used in src/ but missing from the "Valid entity_type values"
        # block — the only declarative list of what the argument may be.
        # Phase 2 (backend — extend the docstring to match the call sites).
        "dag_group",
        "metagen_boundary",
        "metagen_candidate",
        "metagen_item",
...
#: catalogue. These reach clients as a 404 `error_code` no client can look up.
KNOWN_DRIFT_ENTITY_CODE_NOT_IN_API_MD: frozenset[str] = frozenset(
    {
        # Raised in src/ but absent from spec/API.md §Application Error Codes, which does
        # catalogue the sibling DAG_GROUP_NOT_FOUND and TOKEN_NOT_FOUND.
        # Phase 1 (API-centric — add the rows).
        "METAGEN_BOUNDARY_NOT_FOUND",
        "METAGEN_CANDIDATE_NOT_FOUND",
        "METAGEN_ITEM_NOT_FOUND",
        "SEED_NOT_FOUND",
        "USER_NOT_FOUND",
    }
)

#: Disagreements between the module-docstring mapping block in exceptions.py and the
#: per-class docstring blocks in the same file. The module block is a stale third copy
#: of a mapping that spec/feature/BACKEND.md §Exception-to-HTTP Mapping and the class
#: blocks already carry. Phase 2 (backend) resolves the whole set — most cheaply by
#: replacing the duplicate block with a pointer to the two live catalogues.
KNOWN_DRIFT_MODULE_DOCSTRING_BLOCK: frozenset[str] = frozenset(
    {
        "AuthenticationError: class absent from module docstring block",
        "BadRequestError: class absent from module docstring block",
        "ForbiddenError: class absent from module docstring block",
        "ConflictError: EMAIL_ALREADY_REGISTERED absent from module docstring block",
        "ConflictError: EMAIL_BOUND_TO_ANOTHER_GOOGLE_ACCOUNT absent from module docstring block",
        "ConflictError: GOOGLE_ACCOUNT_LINKED_ELSEWHERE absent from module docstring block",
        "ConflictError: GOOGLE_IS_ONLY_AUTH_METHOD absent from module docstring block",
        "ConflictError: INGESTION_RUN_NOT_APPLICABLE absent from module docstring block",
        "ConflictError: INGESTION_SOURCE_READONLY absent from module docstring block",
        "ConflictError: METAGEN_CONF_EXISTS absent from module docstring block",
        "ConflictError: METAGEN_DISABLED absent from module docstring block",
        "ConflictError: METAGEN_RUNNING absent from module docstring block",
        "ConflictError: METRIC_EXISTS absent from module docstring block",
        "ConflictError: TOKEN_LIMIT_EXCEEDED absent from module docstring block",
        "EntityNotFoundError: INGESTION_SOURCE_NOT_FOUND absent from module docstring block",
        "EntityNotFoundError: METAGEN_CONF_NOT_FOUND absent from module docstring block",
        "PreconditionFailedError: METAGEN_DATASET_NOT_IN_BOUNDARY absent from module "
        "docstring block",
        "PreconditionFailedError: SECRET_REF_MALFORMED absent from module docstring block",
        "PreconditionFailedError: SECRET_REF_NOT_FOUND absent from module docstring block",
    }
)
...
class TestCatalogueParsing:
    ...
    def test_module_docstring_mapping_parsed(self) -> None:
        parsed = module_docstring_exception_mapping()
        for expected in ("EntityNotFoundError", "ConflictError", "PreconditionFailedError"):
            assert parsed.get(expected), (
                f"{expected} not parsed from the exceptions.py module docstring block: "
                f"{sorted(parsed)}"
            )
...
class TestModuleDocstringMirrorsClassDocstrings:
    """The mapping block in ``exceptions.py``'s module docstring is a third copy.

    It restates what the per-class docstring blocks and
    ``spec/feature/BACKEND.md §Exception-to-HTTP Mapping`` already say. Duplicated
    catalogues drift; this asserts the copy against the classes it describes, so the
    drift is bounded and visible instead of accumulating silently.
    """

    def test_module_block_matches_class_blocks(self) -> None:
        module_block = module_docstring_exception_mapping()
        class_blocks = exception_docstring_error_codes()
        assert class_blocks, "No class docstring blocks parsed — nothing was compared"
        drift: set[str] = set()
        for class_name, class_codes in class_blocks.items():
            module_codes = module_block.get(class_name)
            if module_codes is None:
                drift.add(f"{class_name}: class absent from module docstring block")
                continue
            for code in class_codes - module_codes:
                drift.add(f"{class_name}: {code} absent from module docstring block")
            for code in module_codes - class_codes:
                drift.add(f"{class_name}: {code} in module docstring block but not on the class")
        assert_drift_allowlist(
            drift,
            KNOWN_DRIFT_MODULE_DOCSTRING_BLOCK,
            what="disagreement between the exceptions.py module-docstring mapping block "
            "and the per-class docstring blocks",
            allowlist_name="KNOWN_DRIFT_MODULE_DOCSTRING_BLOCK",
        )
