...
3. Scan `src/frontend/` to match existing conventions before adding files.
...
- `pnpm -C src/frontend test` (Vitest + Testing Library — colocated specs; this also runs `typecheck` first via the `pretest` script)
...
- **Deferred**: items that need another role (backend API endpoints, shared types, etc.)
