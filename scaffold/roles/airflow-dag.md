...
Run `uv run pytest tests/unit/workflows/` to verify. Run `scaffold/bin/lint-python.sh <file>` (or `uv run ruff check <file>`) on every Python file you touched.
...
- **Deferred**: items that need another role (backend service methods, activity endpoints, etc.)
