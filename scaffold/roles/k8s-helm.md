...
2. Scan `helm-charts/` to match current structure.
...
├── .env.dev.example            # 3-section env file (kube deployment / dev profile inputs / auto-populated dev access)
├── bin/
...
- **Deferred**: items that need another role or manual testing (cluster deployment, ingress endpoint validation)
