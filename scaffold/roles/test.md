...
- Before invoking integration tests, verify the dev cluster is healthy (`helm-charts/bin/health-check.sh`); the repo's `tests/integration/conftest.py::require_server` autouse fixture also asserts server/stub health at session start regardless of which agent or human runs pytest.

## E2E (Playwright / TypeScript)

`tests/e2e/` is a self-contained pnpm/TypeScript project. Authoritative conventions live in
`spec/TESTING.md §End-to-End (E2E) Testing` — read it first. Key points for this role:
...
- **Two groups**: `use-case/` mirrors `tests/integration/api_wired/test_uc{1..5}_*.py` (one
  browser flow per `USE_CASE_en.md` story, narrative annotated verbatim) and manual UC
  walkthroughs; `ground/<feature>/` is the spot analogue — narrow single-concern UI flows filling
  the routes the use-case group doesn't reach. Together they cover every route under
  `src/frontend/app/` (track in `tests/e2e/COVERAGE.md`). Do not duplicate presentational logic
  already in Vitest.
...
- **Selectors**: semantic-first (`getByRole`/`getByLabel`/`getByText`); request a `data-testid`
  from the `frontend` role only where a semantic locator is insufficient (charts, dynamic rows,
...
The prompt includes specific findings from the reviewer role. Write targeted tests that verify or disprove each finding:
...
Run the tests you wrote to verify they pass. Fix any failures before reporting completion. Run
`scaffold/bin/lint-python.sh <file>` (or `uv run ruff check <file>`) on every Python file you touched.
...
- **E2E coverage delta** (when touching `tests/e2e/`): which routes the new tests cover and how `tests/e2e/COVERAGE.md` changed; list any `data-testid` attributes the `frontend` role must add (component + element) for selectors you could not write semantically.
