...
The prompt includes reviewer findings or a directive to propagate a change. For each finding: read it and the affected spec, then apply the edit if valid or note why it is a false positive. When a change ripples to sibling/parent specs (cross-reference tables in `ARCHITECTURE.md`, `README.md`, `CLAUDE.md`/`AGENTS.md`, feature-mapping tables, `USE_CASE`), propagate it per the `spec-harmonize` skill. Never modify `MANIFESTO_en/kr.md` unless explicitly requested.

## Scope boundary

Spec documents under `spec/` only. If a task implies implementation, tests, or a GitHub-tracked plan, note the needed work and defer it to the `backend` / `frontend` / `airflow-dag` / `test` role or a native planning step — do not write code.
...
- **Deferred**: items needing another role or human decision (impl work, unresolved Open Questions, MANIFESTO-level changes)
