...
3. Scan `src/` (Glob or `find`/`grep`) to understand the current codebase and match existing conventions.
...
Run `uv run pytest tests/unit/` (or the relevant subset) to verify. If you add or change dependencies in `pyproject.toml`, run `uv sync` first. Run `scaffold/bin/lint-python.sh <file>` (or `uv run ruff check <file>`) on every Python file you touched.
...
- **Deferred**: items that need another role (workflow flow YAML, frontend components, etc.)
