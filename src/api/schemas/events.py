...
class EventResponse(SingleResponse):
    ...
    entity_type: str = Field(
        description=(
            "Type of the entity that triggered the event, e.g. 'ingestion_config', "
            "'validation_config'"
        )
    )
    ...
    event_type: str = Field(
        description=(
            "Type of the event, e.g. 'run_started', 'run_completed', 'config_updated'"
        )
    )
    status: EventStatus = Field(
        description=(
            "Outcome status of the event: 'success', 'ok', 'failure', 'running', or 'warning'"
        )
    )
    detail: dict[str, Any] = Field(
        default={},
        description=(
            "Additional event details, e.g. error messages, run statistics, or changed fields"
        ),
    )
    ...
    wrapper: bool = Field(
        default=False,
        description=(
            "True when this event is mirrored from a linked DataHub CLI wrapper source "
            "rather than recorded on the regular source itself"
        ),
    )
