...
async def evaluate_rule(
    ...
    try:
        if rule_type == "freshness":
            ...
            return await _evaluate_sql(datahub, dataset_urn, rule, partition)
        else:
            return await _evaluate_custom(datahub, dataset_urn, rule, partition)
...
async def _evaluate_sql(
    ...
    """SQL-based evaluation — not yet implemented."""
    rule_id = rule.get("rule_id", "")
    return RuleEvaluation(
        rule_id=rule_id,
        assertion_result="ERROR",
...
async def _evaluate_custom(
    ...
    """Custom rule evaluation — not yet implemented."""
    rule_id = rule.get("rule_id", "")
    return RuleEvaluation(
        rule_id=rule_id,
        assertion_result="ERROR",
