...
if [[ -n "${DATASPOKE_DEV_INTERNAL_TOKEN:-}" ]]; then
  curl -sS -o /dev/null -X POST "${BASE}/internal/admin/bootstrap" \
    -H "X-Internal-Token: ${DATASPOKE_DEV_INTERNAL_TOKEN}" \
...
INTERNAL_TOKEN=${DATASPOKE_DEV_INTERNAL_TOKEN:-}
...
DATASPOKE_PG_HOST=${DATASPOKE_DEV_POSTGRES_HOST:-}
DATASPOKE_PG_PORT=${DATASPOKE_DEV_POSTGRES_PORT:-}
DATASPOKE_PG_USER=${DATASPOKE_DEV_POSTGRES_USER:-}
DATASPOKE_PG_PASSWORD=${DATASPOKE_DEV_POSTGRES_PASSWORD:-}
DATASPOKE_PG_DB=${DATASPOKE_DEV_POSTGRES_DB:-}
DATASPOKE_K8S_NAMESPACE=${DATASPOKE_KUBE_DATASPOKE_NAMESPACE}
EOF
chmod 600 "$OUT"
...
echo "  PG (dataspoke)= ${DATASPOKE_DEV_POSTGRES_HOST:-?}:${DATASPOKE_DEV_POSTGRES_PORT:-?}"
echo "  k8s ns       = ${DATASPOKE_KUBE_DATASPOKE_NAMESPACE}"
