...
2. If it already exists, verify the canonical variables are present per spec `spec/feature/HELM_CHART.md §Configuration — Four-Tier Env Vars`:
   ...
   - **Test access** (`DATASPOKE_DEV_*`): auto-populated by `install.sh` post-install via `_sync_env_from_secret`; never manually edited. Read by `tests/integration/` for laptop-side cluster access.
...
4. **Prod only** — `install.sh` fails fast on all of the following *before* creating any resource. Check them here and stop with the remedy rather than letting the script abort mid-run (authoritative table: `README.md §2`):
   ...
   - The `DATASPOKE_KUBE_INGRESS_CLASS` IngressClass exists: `kubectl get ingressclass <class>`. Required explicitly in prod with no default (a default would republish on whatever controller happens to be named `nginx`); `nginx` is the dev default. Read which controller serves it with `kubectl get ingressclass <class> -o jsonpath='{.spec.controller}'`.
