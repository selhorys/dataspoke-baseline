...
Internal activity endpoints (`/api/v1/internal/activities/*`) live in `src/api/routers/internal/activities.py` and are **in scope** for this agent — they use `make_*` factory functions from `src/workflows/_common.py` (not FastAPI `Depends()`) and delegate to `src/backend/` services.
