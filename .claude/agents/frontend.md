...
2. Read `src/api/routers/` — the API contract your frontend consumes. Alternatively, check the live ReDoc UI at `http://app.<INGRESS_DOMAIN>/redoc` if the in-cluster API is running (see `dev_env/.env` for the domain).
