...
**How to apply:** When a diff claims two adjacent controls are visually matched, do not compare the authored class strings — resolve specificity. Anything rendered inside `ui/button.tsx` is size-4 regardless of its own `h-*`/`w-*`. To match a Button-hosted icon from a non-Button trigger, use `[&>svg]:size-4`, not `h-3.5`. Cosmetic-only, so this is a low finding, not a blocker; jsdom/Vitest cannot catch it — see [[project_recharts_css_var_svg_attr]] for the same "verify rendering empirically" lesson.
