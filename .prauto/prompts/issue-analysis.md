...
- **Security**: a JSON array naming the subset of Stages whose diff touches the sensitive paths listed in `scaffold/roles/security-reviewer.md` (all of `src/shared/**` and `src/backend/**`, plus `src/api/` auth / middleware / routers, migrations, dependency manifests, helm secrets/values, `.prauto/**`, install-time orchestration). Empty array `[]` if none.
- **Skip-plan eligible**: `yes` only if your plan meets ALL of AGENTS.md's skip-plan criteria — touches < 3 files AND < 60 lines of logic; introduces no new API endpoint, DB table/column, pgvector collection, or Airflow DAG; and requires no cross-layer coordination. Otherwise `no`. Judge your own plan, not the issue's self-description. When unsure, answer `no`.
