...
Bootstrap env:
```bash
bash .agents/skills/test-manual-api-wired/helpers/setup_env.sh
```
...
5. **401 retry**: if HTTP 401 with `error_code=UNAUTHORIZED`, run
   `bash .agents/skills/test-manual-api-wired/helpers/refresh_token.sh`,
...
- `helpers/probes.py` — DB / GMS / k8s side-effect probes. Run as
  `python3 .agents/skills/test-manual-api-wired/helpers/probes.py <probe> <args>`.
