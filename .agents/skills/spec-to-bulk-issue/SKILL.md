...
Use the helper script at `.agents/skills/spec-to-bulk-issue/register-issues.sh` which handles auth verification, title/body parsing, and sequential `gh issue create` calls:

```bash
bash .agents/skills/spec-to-bulk-issue/register-issues.sh <file1> <file2> ...
```
