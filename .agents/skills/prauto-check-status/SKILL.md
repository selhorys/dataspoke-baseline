...
A helper script at `.agents/skills/prauto-check-status/status.sh` automates the full dashboard and next-heartbeat prediction. Run it first:

```bash
.agents/skills/prauto-check-status/status.sh          # full report
.agents/skills/prauto-check-status/status.sh next      # prediction only
.agents/skills/prauto-check-status/status.sh wip       # single section
```
