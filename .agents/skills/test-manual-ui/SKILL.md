...
- `.agents/skills/test-manual-api-wired/helpers/setup_env.sh` — bootstrap env +
  admin JWT into `/tmp/_manual_test_env`.
- `.agents/skills/test-manual-api-wired/helpers/refresh_token.sh` — re-issue the
  admin JWT on 401.
- `.agents/skills/test-manual-api-wired/helpers/probes.py` — DB / GMS / k8s
  side-effect probes. Run `python3 .../probes.py --list` to enumerate.

Own helper:

- `.agents/skills/test-manual-ui/helpers/preflight.sh` — health-check, confirm
...
```bash
bash .agents/skills/test-manual-ui/helpers/preflight.sh
```
